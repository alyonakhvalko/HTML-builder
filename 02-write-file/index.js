...
console.log('Привет! Напиши свой текст:');

stdin.on('data', data => {
    const writeData = data.toString();
    if (writeData.startsWith('exit')) {
        output.close();
        console.log('Ваш текст записан в файл! Завершено удачно (при вводе exit)!')
        process.exit();
    } else {
...
process.on('SIGINT', () => {
    output.close();
    console.log('Ваш текст записан в файл! Завершено удачно (при нажатии ctrl+c)!')
    process.exit();
});
